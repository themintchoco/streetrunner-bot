...
async def gen_card(username: str, card_type: CardType) -> BytesIO:
    ...
    draw_base.text((9 * SPACING + image_skin.width, 6 * SPACING + 24 + 18), player_info.stats_prison.rank,
                   (252, 234, 168), font_stats)

    length_stats_rank = draw_base.textlength(player_info.stats_prison.rank, font_stats)

    draw_base.text((13 * SPACING + image_skin.width + max(length_stats_rank, 100), 5 * SPACING + 24), 'KDA',
                   (127, 127, 127), font_stats_header)
    draw_base.text((13 * SPACING + image_skin.width + max(length_stats_rank, 100), 6 * SPACING + 24 + 18),
                   str(player_info.stats_arena.kda), (252, 234, 168), font_stats)
